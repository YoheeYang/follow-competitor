...
- **COMPETITOR_OFFICIAL (Sumsub news/media + competitor official sources):** After building the listing, `prepare.js` fetches each **canonical article URL** and stores stripped body text as **`articleText`** for the LLM to summarize (not just the listing title/excerpt). Sumsub uses its Next.js post content; Trulioo / Jumio / Veriff use generic blog/RSS discovery with 7-day windows. Persona is intentionally excluded for now because automated fetches return 403. Official items are filtered by confirmed `publishedAt`: if the listing has no date, `prepare.js` tries the detail page JSON-LD/meta/time fields; feeds with `acceptUndated=false` drop still-undated items. The prepare layer also filters official content unrelated to KYC / AML / KYB / IDV / fraud / compliance / regulatory themes.
